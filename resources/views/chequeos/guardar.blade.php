        ...
        <div class="col-6">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="hora_alarma"
                    value="{{ old('hora_alarma') == null ? (isset($chequeo) ? $chequeo->hora_alarma : '') : old('hora_alarma') }}"
                    readonly>
            </div>
            <div class="form-group">
                <label for="fecha_alarma" class= "label">Fecha</label>
                <input type="text" class="form-control" name="fecha_alarma"
                    value="{{ old('fecha_alarma') == null ? (isset($chequeo) ? $chequeo->fecha_alarma : '') : old('fecha_alarma') }}"
                    readonly>
            </div>
            <div class="form-group">
                <label for="codigo" class= "label">Codigo</label>
                <input type="text" class="form-control" name="codigo"
                    value="{{ old('codigo_alarma') == null ? (isset($chequeo) ? $chequeo->codigo_alarma : '') : old('codigo_alarma') }}"
                    readonly>
            </div>
            <div class="form-group">
                <label for="nombre_usuario" class= "label">Usuario</label>
                <input type="text" class="form-control" name="nombre_usuario"
                    value="{{ old('nombre_usuario') == null ? (isset($chequeo) ? $chequeo->nombre_usuario : '') : old('nombre_usuario') }}"
                    readonly>
            </div>
            <div class="form-group">
                <label for="calle" class= "label">Calle</label>
                <input type="text" class="form-control" name="calle"
                    value="{{ old('calle_direccion') == null ? (isset($chequeo) ? $chequeo->calle_direccion : '') : old('calle_direccion') }}"
                    readonly>
            </div>
            <div class="form-group">
                <label for="numero" class= "label">Numero</label>
                <input type="text" class="form-control" name="numero"
                    value="{{ old('numero_direccion') == null ? (isset($chequeo) ? $chequeo->numero_direccion : '') : old('numero_direccion') }}"
                    readonly>
            </div>
        ...
        <div class="col-6">
            ...
            <form method="POST" class="form form__container" action ="{{ route('chequeos.handleGuardar') }}">
                ...
                @if (isset($chequeo))
                    ...
                    <input type="hidden" class="form-control" name="usuario_emisor"
                        value="{{ old('nombre_usuario') == null ? (isset($chequeo) ? $chequeo->nombre_usuario : '') : old('nombre_usuario') }}"
                        readonly>
                @endif
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="tipo_chequeo">
                        @foreach ($tiposChequeo as $tipoChequeo)
                            <option value="{{ $tipoChequeo->id }}"
                                {{ old('tipo_chequeo') == null ? (isset($chequeo) && $chequeo->tipo_chequeo == $tipoChequeo->id ? 'selected' : '') : (old('tipo_chequeo') == $tipoChequeo->id ? 'selected' : '') }}>
                                {{ $tipoChequeo->nombre }}</option>
                        @endforeach
                    ...
                    <select class="form-control" name="tipo_evento">
                        @foreach ($tiposEvento as $tipoEvento)
                            <option value="{{ $tipoEvento->id }}"
                                {{ old('tipo_evento') == null ? (isset($chequeo) && $chequeo->tipo_evento == $tipoEvento->id ? 'selected' : '') : (old('tipo_evento') == $tipoEvento->id ? 'selected' : '') }}>
                                {{ $tipoEvento->nombre }}</option>
                        @endforeach
                ...
                <div class="d-flex justify-content-end py-2">
                    <button type="submit" class="btn btn-primary"
                        style="margin-right: 20px; background: #509fd8;">Guardar</button>
